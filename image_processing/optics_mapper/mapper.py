"""
mapper.py  –  Phase 1: Breadboard grid detection + OCR label reading
              Outputs a JSON component map and an SVG diagram.

Usage (module):
    from optics_mapper.mapper import process_image
    process_image("photo.jpg", debug=True, out_dir=Path("output/"))

Outputs:
    <stem>_map.json   – component positions in breadboard coordinates
    <stem>_map.svg    – visual map overlaid on a rectified top-down view
    <stem>_debug.jpg  – annotated source image (with debug=True)
"""
...
def cluster_labels_into_components(labels: list, grid_info: dict):
    ...
    for cid in set(cluster_ids):
        ...
        merged_text = " / ".join(
            dict.fromkeys(m["text"] for m in members)
        )
...
def build_svg(img_bgr: np.ndarray, holes, grid_info: dict,
    ...
    """
    Produce an SVG overlaying detected holes, OCR boxes, and component badges
...
def process_image(image_path: str, debug: bool = False, out_dir: Path = None):
