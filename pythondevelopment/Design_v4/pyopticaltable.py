...
fontparams = {'fontsize': 7, 'fontweight': 'normal', 'ha': 'center',
...
class OpticalTable:
    ...
    def __init__(self, length, width, size_factor=10.0, edgecolour='k', grid_spacing=1.0,
        ...
        self.ax = plt.gca()
        self.ax.set_aspect('equal')  # Ensure equal scaling of x and y units
    ...
    def transmissive_plate(self, x, y, size, angle, colour='k', offset_factor=0.05, 
        ...
        if fill:
            ...
            rectangle_coords = [
                (coords1[0], coords1[2]),  # bottom-left
                (coords1[1], coords1[3]),  # top-left
                (coords2[1], coords2[3]),  # top-right
                (coords2[0], coords2[2])  # bottom-right
            ]
            poly = mpl.patches.Polygon(rectangle_coords, closed=True,
                                       edgecolor='none', facecolor=fillcolour,
                                       zorder=zorder)
...
class LaserBeam:
    ...
    def draw(self, table, optics):
        ...
        #                       linestyle=self.style)
            ...
            for i in range(len(optics) - 1):
                x0, y0 = optics[i].x, optics[i].y
                x1, y1 = optics[i + 1].x, optics[i + 1].y

                # Draw the full line as a thin guide
                table.ax.plot([x0, x1], [y0, y1],
                              color=self.colour,
                              linewidth=self.width * 0.5,
                              linestyle=self.style)

                # Calculate midpoint and unit vector
                ...
                arrow_length = 0.4
                table.ax.arrow(
                    midx - dx_unit * (arrow_length / 2), midy - dy_unit * (arrow_length / 2),  # new tail
                    dx_unit * arrow_length, dy_unit * arrow_length,  # new vector
                    head_width=0.1,
                    head_length=0.2,
                    fc=self.colour, ec=self.colour,
                    linewidth=self.width,
                    linestyle=self.style,
